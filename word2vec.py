...
for index, item in tqdm(s.iterrows()):
    ...
    new_words_vec = np.array([final_embeddings[dictionary[w]] for w in new_words], dtype=np.float32).reshape(1,32*128)
    vecs += [new_words_vec]
...
def cnn_model_fn(features, labels, mode):
  ...
  input_layer = tf.reshape(features, [-1, 128, 32, 1])
  ...
  # Input Tensor Shape: [batch_size, 32 * 8 * 64]
...
def main(unused_argv):
  ...
  # train_data_m = mnist.train.images  # Returns np.array
  # train_labels_m = np.asarray(mnist.train.labels, dtype=np.int32)
  # eval_data_m = mnist.test.images  # Returns np.array
  # eval_labels_m = np.asarray(mnist.test.labels, dtype=np.int32)
